...
1. Client-side validation runs first (required name, company, email, category, description, consent).
2. A hidden `website` field is a honeypot.
3. If `CONTACT_WEBHOOK_URL` is unset, the API validates the payload and returns success without storing files. This is the mock handler.
4. If the webhook is set, the API forwards JSON (without the honeypot) to that URL.
...
- Attachments: the mock handler does not persist files. Extend the route if you need object storage.
...
3. Set `CONTACT_WEBHOOK_URL` if the form should leave the mock handler.
4. Run `npm run build`.
5. Host with any Node-compatible platform that can run `next start`, or a provider with a Next.js adapter.
